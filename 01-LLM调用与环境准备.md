...
- 把一次裸调用封装成最小的 `LLMClient`，为后面所有章节（ReAct、RAG、Runtime）打地基——这就是我们「从零手写 Harness」的第一块砖。
...
**官方文档与资源**：详见 [工具导航与参考资料索引](工具导航与参考资料索引.md#llm-与-sdk)。
...
行业正在发生一个重要变化（详见你自己的[面试八股](面试题库.md#七、前沿主线)）：**Agent 的工程重心已经从 Prompt 迁移到 context、harness、eval、protocol、runtime。** 也就是说，真正值钱的能力不是「会调框架」，而是「看懂 LLM 在一个什么脚手架（Harness）里工作」。
...
完整代码见 [案例与源码-1-Prompt层/llm_client.py](案例与源码-1-Prompt层/llm_client.py)。核心长这样：
...
预期输出：先打印同步调用的完整回答，再以流式方式逐字打印第二段回答。
