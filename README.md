...
![Chapters](https://img.shields.io/badge/已发布-01·02·05·06·07·08·09·10·11·12·13·14-09f?style=flat)

> 📌 **当前进度**：已发布 `01`（LLM 调用）、`02`（消息状态）、`05`（工具循环）、`06`（工具容错）、`07`（结构化输出）、`08`（Context 治理）、`09`（记忆体系）、`10`（任务拆解与 Planning）、`11`（失败策略与反思）、`12`（Checkpoint 与状态恢复）、`13`（封装 Mini Agent Runtime）、`14`（可观测：Trace/日志/回放）——「调用 → 状态 → 循环 → 容错 → 结构化 → 治理 → 记忆 → 规划 → 反思 → 保进度 → 封 runtime → 可观测」主线十二连，阶段五 Runtime 层推进中；其余章节规划中，侧边栏已标注，点开不会 404。
