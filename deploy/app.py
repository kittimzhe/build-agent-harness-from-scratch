...
def _build_agent() -> DeepResearchAgent:
    """每个请求建一个独立 agent（无状态服务，避免跨用户串记忆/检查点）。"""
    if os.getenv("DEEP_RESEARCH_OFFLINE") == "1":
        ...
        return DeepResearchAgent(llm=llm, engine=FakeSearchEngine(),
                                 workdir=os.getenv("WORKDIR", ".deep_research"))
    return DeepResearchAgent(llm=LLMClient(),
                             workdir=os.getenv("WORKDIR", ".deep_research"))


@app.get("/health")
def health() -> dict:
    """存活探针：负载均衡 / Docker HEALTHCHECK 用。"""
